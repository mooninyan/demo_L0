...
func (h *Handler) getOrderByID(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	ord, err := h.orderService.GetByID(timeout, id)
	if err != nil {
		log.Println("getOrderByID() error:", err)
...
func (h *Handler) createOrder(w http.ResponseWriter, r *http.Request) {
	...
	var ord order.MainOrder
	if err = json.Unmarshal(reqBytes, &ord); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	timeout, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	err = h.orderService.Create(timeout, &ord)
	if err != nil {
...
func (h *Handler) getAllOrders(w http.ResponseWriter, _ *http.Request) {
